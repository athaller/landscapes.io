...
function seedRole(role){
  ...
  return new Promise(function (reject,resolve){
    ...
    Role.find({ name: role.name }, function (err, roles) {
      if (err) {
        reject(new Error('Failed to find local account ' + role.name));
      }

      if (roles.length === 0) {
        role.save(function (err, savedRole) {
          if (err) {
            reject(new Error('Role Error saving ' + role.name ));
          } else {
            console.log(chalk.bold.red('Database Seeding:\t\t\tRole ' + role.name +' Added' ));
            resolve(role);
          }
        });
      } else {
        console.log(chalk.bold.red('Database Seeding:\t\t\tRole ' + role.name +' already added' ));
        resolve(role);
      }
    });
  });
...
function seedTheUser (user, role) {
...
module.exports.start = function start(options) {
  ...
  return new Promise(function (resolve, reject) {
    ...
    //Default Roles stored in main config
    var adminRole = new Role(config.defaultRoles[0]);
    ...
    if (process.env.NODE_ENV === 'production') {
      ...
      User.generateRandomPassphrase()
        .then(seedRole(userRole))
        .then(seedTheUser(userAccount,userRole ))
        .then(User.generateRandomPassphrase)
        .then(seedRole(adminRole))
        .then(seedTheUser(adminAccount, adminRole))
        .then(function () {
          resolve();
        })
        .catch(reportError(reject));
